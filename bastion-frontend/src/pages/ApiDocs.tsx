...
export default function ApiDocs() {
  ...
  const MethodBadge = ({ method }: { method: 'GET' | 'POST' | 'PUT' | 'DELETE' | 'PATCH' }) => {
    const color = method === 'GET' ? 'text-emerald-400 border-emerald-400/40'
      : method === 'POST' ? 'text-sky-400 border-sky-400/40'
      : method === 'PUT' ? 'text-amber-400 border-amber-400/40'
      : method === 'PATCH' ? 'text-fuchsia-400 border-fuchsia-400/40'
      : 'text-rose-400 border-rose-400/40';
    return <span className={`inline-block text-xs px-2 py-0.5 border rounded ${color}`}>{method}</span>;
  };

  const Chip = ({ label, kind }: { label: string; kind: 'curl' | 'python' | 'js' | 'http' }) => {
    ...
    return <span className={`inline-block text-[10px] leading-4 px-2 py-0.5 rounded ${map[kind]}`}>{label}</span>;
  };

  const CodeBlock = ({ code, lang }: { code: string; lang: 'bash' | 'json' | 'python' | 'javascript' }) => {
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription className="text-muted-foreground">
            Bastion SHIELD API reference and guides. For the complete Markdown reference, see <code>API_DOCS.md</code> in the repository root.
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <section id="introduction" className="scroll-mt-24">
              <div className="grid grid-cols-1 gap-6 md:grid-cols-12">
                <div className="md:col-span-5 space-y-3">
                  <h3 className="text-xl font-semibold text-foreground">Introduction & Core Concepts</h3>
                  <p>
                    Bastion helps e‑commerce stores detect and prevent return fraud. The SHIELD API lets you submit
                    return claims for fraud analysis and retrieve a user’s lifetime claim history across integrated
                    stores.
                  </p>
                  <p>
                    <strong>Base URL:</strong> <code>https://api.shield.com/v1</code>
                  </p>
                  <p>
                    <strong>Find or Create workflow:</strong> On claim submission, SHIELD finds a master <code>User</code>
                    by <code>kyc_email</code> (or creates one), then finds or creates a <code>StoreAccount</code> for that
                    user and <code>store_id</code>, computes a risk score, and records the <code>Claim</code>.
                  </p>
                </div>
                <aside className="md:col-span-7 md:pl-6 md:sticky md:top-16">
                  <div className="rounded-lg border bg-card p-4">
                    <div id="intro-curl" className="mt-0">
                      <div className="flex items-center gap-2 mb-1"><Chip label="curl" kind="curl" /><span className="text-xs text-muted-foreground">example</span></div>
                      <CodeBlock lang="bash" code={`curl -X GET "https://api.shield.com/v1/health" \\
            ...
            <section id="authentication" className="scroll-mt-24">
              <div className="grid grid-cols-1 gap-6 md:grid-cols-12">
                <div className="md:col-span-5 space-y-3">
                  ...
                  <p>
                    Authenticate by passing your API key via the <code>X-API-Key</code> header. All requests must use
                    HTTPS. Requests without authentication or using HTTP will fail with <code>401</code>.
                  </p>
                </div>
                <aside className="md:col-span-7 md:pl-6 md:sticky md:top-16">
                  <div className="rounded-lg border bg-card p-4">
...
-H "X-API-Key: <YOUR_API_KEY>"`} />
                    ...
                    <p className="mt-3">Missing or invalid keys return <code>401 Unauthorized</code>.</p>
                  </div>
                </aside>
              </div>
            </section>
            ...
            <section id="data-models" className="scroll-mt-24">
              <div className="grid grid-cols-1 gap-6 md:grid-cols-12">
                <div className="md:col-span-5 space-y-3">
                  <h3 className="text-xl font-semibold text-foreground">Data Models / Schemas</h3>
                  <p><strong>User</strong>: master, KYC‑verified identity.</p>
                  <ul className="list-disc pl-5">
                    <li><code>id</code> (uuid), <code>kyc_email</code> (string), <code>full_name</code> (string), <code>dob</code> (YYYY‑MM‑DD)</li>
                    <li><code>risk_score</code> (0–100), <code>is_flagged</code> (boolean), <code>created_at</code> (ISO 8601)</li>
                  </ul>
                  <p><strong>StoreAccount</strong>: user’s account at a single store.</p>
                  <ul className="list-disc pl-5">
                    <li><code>id</code> (uuid), <code>user_id</code> (uuid), <code>store_id</code> (uuid), <code>email_at_store</code> (string)</li>
                  </ul>
                  <p><strong>ItemData</strong>: item within a claim.</p>
                  <ul className="list-disc pl-5">
                    <li><code>item_name</code> (string), <code>category</code> (string), <code>price</code> (float), <code>quantity</code> (int), <code>url</code> (string?)</li>
                  </ul>
                  <p><strong>Claim</strong>: primary claim object.</p>
                  <ul className="list-disc pl-5">
                    <li><code>id</code> (uuid), <code>store_account_id</code> (uuid), <code>status</code> ("PENDING"|"APPROVED"|"DENIED"), <code>claim_data</code> (ItemData[]), <code>created_at</code> (ISO 8601)</li>
                  </ul>
                </div>
                <aside className="md:col-span-7 md:pl-6 md:sticky md:top-16">
                  <div className="rounded-lg border bg-card p-4">
                    <div className="text-foreground font-medium mb-2">Example User object</div>
  ...
  "created_at": "2023-05-01T12:00:00Z"
            ...
            <section id="post-claims" className="scroll-mt-24">
              <div className="grid grid-cols-1 gap-6 md:grid-cols-12">
                <div className="md:col-span-5 space-y-3">
                  <h3 className="text-xl font-semibold text-foreground">POST /claims</h3>
                  <p>
                    Submit a new return claim. SHIELD performs Find‑or‑Create for <code>User</code> and
                    <code> StoreAccount</code>, computes risk, and records the <code>Claim</code>.
                  </p>
                  ...
                  <div className="rounded-lg border bg-card p-4 space-y-3">
                    <Tabs defaultValue="post-claims-curl">
                      <TabsList className="flex flex-wrap gap-2">
                        ...
                        <TabsTrigger value="post-claims-response">Response</TabsTrigger>
                      </TabsList>
                      <TabsContent value="post-claims-curl" id="post-claims-curl" className="mt-3">
                        <div className="flex items-center gap-2 text-foreground font-medium"><Chip label="curl" kind="curl" /></div>
                        <CodeBlock lang="bash" code={`curl -X POST "https://api.shield.com/v1/claims" \\
...
+-d '{
  ...
  "claim_context": {"store_id": "8765-uuid-of-store", "email_at_store": "jane.doe.promo@outlook.com", "claim_data": [{"item_name": "iPhone 16 Max", "category": "Electronics", "price": 1299.99, "quantity": 1, "url": "https://store.example.com/product/123"}]}
                      ...
                      <TabsContent value="post-claims-python" id="post-claims-python" className="mt-3">
                        <div className="flex items-center gap-2 text-foreground font-medium"><Chip label="Python" kind="python" /><span className="text-xs text-muted-foreground">requests</span></div>
...
print(r.json())`} />
                      ...
                      <TabsContent value="post-claims-js" id="post-claims-js" className="mt-3">
                        <div className="flex items-center gap-2 text-foreground font-medium"><Chip label="JavaScript" kind="js" /><span className="text-xs text-muted-foreground">fetch</span></div>
                        <CodeBlock lang="javascript" code={`const BASE_URL = "https://api.shield.com/v1";
...
fetch(BASE_URL + '/claims', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-API-Key': API_KEY }, body: JSON.stringify(payload) })
  ...
  .then(json => console.log(json));`} />
                      ...
                      <TabsContent value="post-claims-response" id="post-claims-response" className="mt-3">
                        <div className="text-foreground font-medium">Example response (201)</div>
                        <CodeBlock lang="json" code={`{
  ...
  "created_at": "2025-09-13T23:00:12Z"
            ...
            <section id="get-users-kyc_email" className="scroll-mt-24">
              <div className="grid grid-cols-1 gap-6 md:grid-cols-12">
                <div className="md:col-span-5 space-y-3">
                  <h3 className="text-xl font-semibold text-foreground">GET /users/{'{'}kyc_email{'}'}</h3>
                  <p>Retrieve a complete master user profile, including all <code>StoreAccount</code>s and their <code>Claim</code>s.</p>
                  <p className="font-medium text-foreground">Path parameter</p>
                  <ul className="list-disc pl-5">
                    <li><code>kyc_email</code> (string, required): URL‑encoded KYC email of the user</li>
                  </ul>
                </div>
                <aside className="md:col-span-7 md:pl-6 md:sticky md:top-16">
                  <div className="rounded-lg border bg-card p-4 space-y-3">
                    <Tabs defaultValue="get-users-curl">
                      <TabsList className="flex flex-wrap gap-2">
                        ...
                        <TabsTrigger value="get-users-response">Response</TabsTrigger>
                      </TabsList>
                      <TabsContent value="get-users-curl" id="get-users-curl" className="mt-3">
                        <div className="flex items-center gap-2 text-foreground font-medium"><Chip label="curl" kind="curl" /></div>
                        <CodeBlock lang="bash" code={`kyc_email="jane.d.doe@gmail.com"
...
fetch(BASE_URL + '/users/' + kycEmail, { headers: { 'X-API-Key': API_KEY } })
  ...
  .then(json => console.log(json));`} />
                      ...
                      <TabsContent value="get-users-response" id="get-users-response" className="mt-3">
                        <div className="text-foreground font-medium">Example response (200)</div>
                        <CodeBlock lang="json" code={`{
            ...
            <section id="error-handling" className="scroll-mt-24">
              <div className="grid grid-cols-1 gap-6 md:grid-cols-12">
                <div className="md:col-span-5 space-y-3">
                  ...
                  <p>Errors return JSON with a single <code>detail</code> field.</p>
                  <ul className="list-disc pl-5">
